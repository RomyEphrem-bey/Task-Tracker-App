...
|---|---|---|---|---|
| 1 | Unbounded task/comment *count* (in-memory storage growth) still permits denial of service. Per-field/tag length and count limits are fixed. | Medium (narrowed) | Romy | Add request-size limits and pagination if this API is ever exposed beyond local/course use. |
